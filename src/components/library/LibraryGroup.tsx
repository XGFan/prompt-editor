...
export const LibraryGroup = forwardRef<HTMLDivElement, LibraryGroupProps>(function LibraryGroup({
  group,
  prompts,
  searchQuery,
  readOnly = false,
  ...
  const handleCreatePrompt = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (readOnly) return;
    createPrompt({ area: 'library', groupId: group.id, content: '' });
    if (group.collapsed) {
      toggleGroupCollapsed({ area: 'library', groupId: group.id });
    }
  };
  ...
  const handleDeleteGroup = (e: React.MouseEvent) => {
    ...
    if (readOnly) return;
    if (deleteConfirm) {
      deleteGroup({ area: 'library', groupId: group.id });
  ...
  const handleRenameSave = () => {
    ...
    if (trimmed && trimmed !== group.name) {
      try {
        renameGroup({ area: 'library', groupId: group.id, name: trimmed });
      } catch (error) {
  ...
  const visiblePrompts = searchQuery 
    ? prompts.filter(p => p.content.toLowerCase().includes(searchQuery.toLowerCase()))
    : prompts;
  ...
  return (
    <div 
      ...
      data-testid={`library-group-${group.name}`}
    >
      <div 
      ...
      >
        {dragHandle ? dragHandle : (
          <div
            ...
            data-testid={`library-drag-handle-group-${group.id}`}
          >
        ...
        <div
          ...
          onClick={() => !isRenaming && toggleGroupCollapsed({ area: 'library', groupId: group.id })}
          onKeyDown={(e) => {
      ...
      {!group.collapsed && (
        <div className="flex flex-col gap-0.5 px-2 pb-2 min-h-[4px]">
          <SortableContext items={visiblePrompts.map(p => p.id)} strategy={verticalListSortingStrategy}>
